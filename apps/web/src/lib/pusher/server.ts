...
/**
 * Trigger waktu tampil update event (from admin stopwatch to juri).
 * Channel: `event-{eventId}`, Event: `waktu-tampil-update`
 */
export async function triggerWaktuTampilUpdate(eventId: string, data: { gelanggang_id: string; peserta_id: string; waktu_detik: number }) {
  const pusher = getPusher()
  if (!pusher) return
  await pusher.trigger(`event-${eventId}`, 'waktu-tampil-update', data)
}
